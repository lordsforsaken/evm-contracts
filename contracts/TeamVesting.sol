...
contract VestingContract {
     ...
     * @param _durationUnits The units of the duration(0 = days, 1 = weeks, 2 = months, 3 = minutes)
    ...
    function release(address _beneficiary) external {
        ...
        for (uint256 i = 0; i < schedulesLength; i++) {
            ...
            require(amountToSend > 0, "Cannot release zero");
    ...
    function vestedAmount(VestingSchedule memory _schedule) public view returns (uint256) {
        ...
        } else if (_schedule.durationUnits == DurationUnits.Minutes) {
            sliceInSeconds = 1 minutes;
        }
        if (block.timestamp < _schedule.start) {
            return 0;
